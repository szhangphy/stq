...
def build_report(payload: dict[str, Any]) -> str:
    ...
    return textwrap.dedent(
        ...
        `{translation['current_runtime_unknown_ordering']}`.
        Its physical point-space part is the first 34 rows
        `{translation['current_point_row_ordering']}`,
        while `{translation['synthetic_boundary_rows']}` are synthetic boundary rows used only to close the current compatibility graph.

        The 34 physical rows decompose as:
        `{translation['current_point_block_counts']}`.
        ...
        `{translation['external_standard_row_ordering']}`.
        ...
        stored in `{PROJECTION_SUMMARY_JSON.name}` as `projection_matrix_bs_to_standard_rows`.
        ...
        1. it maps a common 13-generator current AI basis to the external ordinary 13-generator basis anchored by
           `{payload['common_ai_basis_generator_ids']}`;
        ...
        Single:
        - raw `rank(BS)` = `{single['raw_bs_rank']}`
        - raw `rank(AI)` = `{single['raw_ai_rank']}`
        - final `rank(BS)` = `{single['final_rank_bs']}`
        - final `rank(AI)` = `{single['final_rank_ai']}`
        - final quotient = `{single['quotient_group']}`

        Double:
        - raw `rank(BS)` = `{double['raw_bs_rank']}`
        - raw `rank(AI)` = `{double['raw_ai_rank']}`
        - final `rank(BS)` = `{double['final_rank_bs']}`
        - final `rank(AI)` = `{double['final_rank_ai']}`
        - final quotient = `{double['quotient_group']}`
...
def build_next_step_prompt(payload: dict[str, Any]) -> str:
    return textwrap.dedent(
        ...
        Any follow-up should review the explicit projection contract, the three killed free directions, and the downstream stage-2 outputs that now cite final BS = 13.
        """
    ).strip()
...
def generate_outputs(
    ...
    payload = {
        ...
        "artifacts": {
            "current_point_snapshot_json": str(CURRENT_POINT_SNAPSHOT_JSON),
            "row_translation_json": str(ROW_TRANSLATION_JSON),
            "projection_summary_json": str(PROJECTION_SUMMARY_JSON),
            "final_closeout_report_md": str(FINAL_CLOSEOUT_REPORT_MD),
            "final_closeout_status_json": str(FINAL_CLOSEOUT_STATUS_JSON),
            "final_closeout_next_step_prompt_txt": str(FINAL_CLOSEOUT_NEXT_STEP_PROMPT_TXT),
        },
    }
