#!/usr/bin/env python3
"""Legacy SG194-specific target-row projection backend.

This file is historical provenance only. Its projection contract is not the
authoritative validator for the current source-layer representation or
compatibility logic. Keep it for compare/discrepancy reporting only.
"""
...
LEGACY_ROLE = "historical_non_authoritative_compare_only"
...
def generate_outputs(
    ...
    payload = {
        ...
        "interpretation_warning": (
            "This is a legacy externally anchored current-to-standard elimination contract, not an authoritative internal ambient row-space identity proof. "
...
def validate_outputs() -> None:
    ...
    if payload.get("legacy_role") != LEGACY_ROLE:
        raise RuntimeError("standard-space projection summary is not marked legacy/non-authoritative")
    if payload.get("authoritative", True):
        raise RuntimeError("standard-space projection summary still claims authoritative status")
