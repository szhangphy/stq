...
def _continuation_fingerprints_for_line(
    ...
    *,
    ...
    for endpoint in line_obj["endpoints"]:
        ...
        for rep_index, restricted_vector in enumerate(line_vectors, start=1):
            rep_id = f"{endpoint_id}_R{rep_index}"
            fingerprints[rep_id] = {
                "fingerprint_kind": "line_restriction_plus_instance_star_continuation_v3",
                "field": field,
                "line_restricted_vector": _normalized_complex_json_vector(restricted_vector),
                "line_basis_decomposition": _line_decomposition_signature(decompositions[rep_id]),
...
def build_line_block_from_restriction_classes(
    line_obj: dict[str, Any],
    captures: dict[str, Any],
    *,
    ...
    extra_fingerprints = _continuation_fingerprints_for_line(
        line_obj,
        captures,
        coarse_block,
        field=field,
    ...
    for endpoint_entry in endpoint_entries:
        ...
        restriction_classes_by_endpoint[endpoint_id] = classes
        for class_index, entry in enumerate(classes, start=1):
            ...
            for rep_id in entry["rep_ids"][1:]:
                refinement_equations.append(
                    {
                        "basis_id": f"{line_obj['id']}_restriction_class_{endpoint_id}_{class_index:02d}",
                        "terms": [
                            {"unknown": anchor, "coeff": 1, "side": "restriction_class"},
                            {"unknown": rep_id, "coeff": -1, "side": "restriction_class"},
                        ],
                        "restriction_class_rep_ids": list(entry["rep_ids"]),
                        "endpoint_id": endpoint_id,
                        "fingerprint": entry.get("extra_fingerprint"),
                    }
                )
    local_index = {unknown: index for index, unknown in enumerate(coarse_block["local_unknown_ordering"])}
    refinement_rows = []
    for equation in refinement_equations:
    ...
    return {
        **coarse_block,
        "status": "success",
        "equations": list(coarse_block["equations"]) + refinement_equations,
        "matrix_rows": list(coarse_block["matrix_rows"]) + refinement_rows,
        "line_group_signature": {
        ...
        "restriction_class_builder": {
            "status": "primary",
            "builder_kind": "restriction_class_primary_with_instance_star_continuation_v3",
            "field": field,
            "containing_planes": list(line_obj.get("containing_planes", [])),
            "restriction_classes_by_endpoint": restriction_classes_by_endpoint,
            "refinement_row_count": len(refinement_equations),
            "coarse_equation_count": len(coarse_block["equations"]),
            "total_equation_count": len(coarse_block["equations"]) + len(refinement_equations),
        },
        "phase_aware_refinement": {
...
def build_line_block(
    ...
    if phase_aware_profile not in (None, "legacy"):
        return build_line_block_coarse(
            line_obj,
            captures,
            phase_aware_profile=phase_aware_profile,
        )
    return build_line_block_from_restriction_classes(
        line_obj,
        captures,
        field="linear_character",
    )
