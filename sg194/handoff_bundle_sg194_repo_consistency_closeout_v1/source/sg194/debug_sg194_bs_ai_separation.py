...
CURRENT_REPO_POINT_BLOCK_IDS = ["P1", "P2", "P3", "P4", "P5", "P6"]
LEGACY_SELECTION_BLOCKS = [("P1", "GM"), ("P2", "A"), ("P3", "K"), ("P5", "M"), ("P6", "L"), ("B1", "H")]
LEGACY_STATUS_EXPLANATION = (
    "This script remains a legacy/stale reference. Its internal selection surface still follows the older "
    "P1/P2/P3/P5/P6/B1 block choice and therefore must not be treated as the current 194.1.1.1 point-space audit, "
    "whose live snapshot uses P1 through P6. It is retained only to preserve the historical BS-vs-AI object-separation diagnosis."
)
CURRENT_AUTHORITATIVE_REFERENCE_OUTPUTS = [
...
CURRENT_AUTHORITATIVE_REFERENCE_NOTE = (
    "Current authoritative SG194 evidence now lives in the stage2 closeout outputs, not in this legacy/stale BS-vs-AI separation package."
)
...
POINT_BLOCKS = LEGACY_SELECTION_BLOCKS
...
BACKGROUND_FILES = [
    ...
    COMMON_ROOT / "swyckoff_r.py",
    COMMON_ROOT / "swyckoff_k.py",
    COMMON_ROOT / "SSGReps.py",
    COMMON_ROOT / "SG_utils.py",
    COMMON_ROOT / "rep_utils.py",
]
...
def build_summary_json(single_ai: dict[str, Any], single_bs: dict[str, Any], double_ai: dict[str, Any], double_bs: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "current_repo_point_block_ids": CURRENT_REPO_POINT_BLOCK_IDS,
        "legacy_internal_selection_blocks": [item[0] for item in LEGACY_SELECTION_BLOCKS],
        "status_explanation": LEGACY_STATUS_EXPLANATION,
        "current_authoritative_reference_outputs": CURRENT_AUTHORITATIVE_REFERENCE_OUTPUTS,
        "current_authoritative_reference_note": CURRENT_AUTHORITATIVE_REFERENCE_NOTE,
        "active_current_evidence": False,
        "path_contract": {
            "repo_root": str(REPO_ROOT),
            "common_root": str(COMMON_ROOT),
            "swyckoff_r_path": str(COMMON_ROOT / "swyckoff_r.py"),
            "swyckoff_k_path": str(COMMON_ROOT / "swyckoff_k.py")
        },
...
def build_audit_md(summary: dict[str, Any]) -> str:
    return "\n".join(
        [
            ...
            f"- active_current_evidence: `{summary['active_current_evidence']}`",
            f"- current repo point blocks: `{', '.join(summary['current_repo_point_block_ids'])}`",
            f"- legacy internal selection blocks: `{', '.join(summary['legacy_internal_selection_blocks'])}`",
            f"- status explanation: {summary['status_explanation']}",
            "",
            "## Previous Mis-attribution",
...
def build_handoff(summary: dict[str, Any]) -> str:
    return "\n".join(
        [
            ...
            f"- Current authoritative note: {summary['current_authoritative_reference_note']}",
            f"- Current repo point blocks: `{', '.join(summary['current_repo_point_block_ids'])}`",
            f"- Legacy internal selection blocks used here: `{', '.join(summary['legacy_internal_selection_blocks'])}`",
            ...
            *[f"  - `{path}`" for path in summary["current_authoritative_reference_outputs"]],
            "- Read first:",
...
def build_next_step_prompt(summary: dict[str, Any]) -> str:
    return textwrap.dedent(
        ...
        不换群，不扩 workflow。当前这份 `debug_sg194_bs_ai_separation.py` 输出只能当 legacy/stale reference。

        不要把它当 current repo snapshot evidence，因为：
        - current repo point blocks are {', '.join(summary['current_repo_point_block_ids'])}
        - this script still uses legacy internal selection blocks {', '.join(summary['legacy_internal_selection_blocks'])}
        - its honest role is historical object-separation diagnosis only
        - current authoritative outputs now live at {', '.join(summary['current_authoritative_reference_outputs'])}

        已有且不要重做的 legacy reference outputs：
        - `sg194_single_ai_vs_external.json`
        - `sg194_single_bs_vs_external.json`
        - `sg194_double_ai_vs_external.json`
        - `sg194_double_bs_vs_external.json`
        ...
        如果下一轮真要继续这个方向，唯一诚实的做法是：
        - 要么基于 current P1..P6 point-space 重建这套分析
        - 要么继续把它保留为 legacy/stale reference，不再当 active evidence
        """
    ).strip()
...
def build_report_md(summary: dict[str, Any], single_ai: dict[str, Any], single_bs: dict[str, Any], double_ai: dict[str, Any], double_bs: dict[str, Any]) -> str:
    return textwrap.dedent(
        ...
        - active_current_evidence: `{summary['active_current_evidence']}`
        - current authoritative note: {summary['current_authoritative_reference_note']}
        - current repo point blocks: `{', '.join(summary['current_repo_point_block_ids'])}`
        - legacy internal selection blocks used here: `{', '.join(summary['legacy_internal_selection_blocks'])}`
        - status explanation: {summary['status_explanation']}
        - current authoritative outputs to cite instead: `{', '.join(summary['current_authoritative_reference_outputs'])}`
...
def create_package() -> None:
    ...
    readme = textwrap.dedent(
        ...
        - analysis_status: `{ANALYSIS_STATUS}`
        - current repo point blocks: `{', '.join(CURRENT_REPO_POINT_BLOCK_IDS)}`
        - legacy internal selection blocks used here: `{', '.join(item[0] for item in LEGACY_SELECTION_BLOCKS)}`
        ...
        - current authoritative note: {CURRENT_AUTHORITATIVE_REFERENCE_NOTE}
        - current authoritative outputs:
          - `{CURRENT_AUTHORITATIVE_REFERENCE_OUTPUTS[0]}`
          - `{CURRENT_AUTHORITATIVE_REFERENCE_OUTPUTS[1]}`
          - `{CURRENT_AUTHORITATIVE_REFERENCE_OUTPUTS[2]}`
          - `{CURRENT_AUTHORITATIVE_REFERENCE_OUTPUTS[3]}`
          - `{CURRENT_AUTHORITATIVE_REFERENCE_OUTPUTS[4]}`
...
def validate() -> None:
    ...
    assert summary["analysis_status"] == ANALYSIS_STATUS
    assert summary["active_current_evidence"] is False
...
def main() -> None:
    ...
    write_json(
        CURRENT_STATUS_JSON,
        {
            ...
            "blocker": "This audit is legacy/stale and must not be treated as current SG194 snapshot evidence until it is rebuilt on the current P1..P6 point-space.",
            "next_step": "Either rebuild the analysis on the current P1..P6 point-space or keep this file archived as legacy reference only.",
        },
    )
