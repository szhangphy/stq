...
def _require_spglib(caller: str):
    try:
        return import_module("spglib")
    except ImportError as exc:
        raise ImportError(
            f"spglib is required when calling {caller} in common/SSGReps/SSGReps/SG_utils.py, "
            "but importing this module itself should not require spglib."
        ) from exc
...
def identify_SG_from_cell(rotP_list, tauP_list, latt_prim, trig_hexag_latt=False):
    ...
    spglib_mod = _require_spglib("identify_SG_from_cell")
   ...
   #gid, gid_label = identify_SG_from_symmetry(rotP_list, tauP_list)
    ...
    dataset = spglib_mod.get_symmetry_dataset(cell)
...
def identify_SG_from_symmetry(rot_list, tau_list):
    ...
    spglib_mod = _require_spglib("identify_SG_from_symmetry")
    hall_num = spglib_mod.get_hall_number_from_symmetry(rot_list, tau_list, symprec=1e-5)
    sg_info = spglib_mod.get_spacegroup_type(hall_num)
    if sg_info == None:
        print('spglib Fail! Return None!')
        return None, None

    gid, gid_label = sg_info['number'], sg_info['international_short']
    sym_spglib = spglib_mod.get_symmetry_from_database(hall_num)
...
def test():
    spglib_mod = _require_spglib("test")
   ...
   #tau_list = [np.array([0,0,0]), np.array([0,0,1/2]), np.array([0,0,1/2]), np.array([0,0,0])]
    ...
    hall_num = spglib_mod.get_hall_number_from_symmetry(rot_list_conv, tau_list_conv, symprec=1e-5)
    sg_info = spglib_mod.get_spacegroup_type(hall_num)
    gid = sg_info['number']
    sym_spglib = spglib_mod.get_symmetry_from_database(hall_num)
...
def test_hall():
    spglib_mod = _require_spglib("test_hall")
