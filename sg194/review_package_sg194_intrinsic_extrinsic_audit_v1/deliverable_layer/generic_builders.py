    ...
    *,
        ...
        line_builder_compare.append(
            {
                "line_id": line["id"],
                "builder_variant": builder_variant,
                "primary_builder_kind": block.get("restriction_class_builder", {}).get("builder_kind", builder_variant),
    ...
    return {
        ...
        "compatibility_builder_kind": {
            "coarse": "generic_coarse_basis_decomposition_line_builder",
            "intrinsic": "generic_intrinsic_line_restriction_class_builder",
            "extrinsic": "generic_extrinsic_star_augmented_line_restriction_class_builder",
        }[builder_variant],
...
@lru_cache(maxsize=None)
def generic_mode_bundle(group_id: str, mode: str, builder_variant: str = "intrinsic") -> dict[str, Any]:
    if mode not in {"single", "double"}:
        raise ValueError(f"unsupported generic mode: {mode}")
    if builder_variant not in {"coarse", "intrinsic", "extrinsic"}:
...
def generic_result_objects(group_id: str, builder_variant: str = "intrinsic") -> list[dict[str, Any]]:
    results = []
        ...
        results.append(
            {
                ...
                "dbs_dai_gap": quotient["dbs_dai_gap"],
