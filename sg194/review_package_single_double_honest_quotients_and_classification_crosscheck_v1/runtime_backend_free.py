...
PACKAGE_NAME = "review_package_single_double_honest_quotients_and_classification_crosscheck_v1"
...
def build_double_pilot(
    ...
    *,
    ...
    double_authoritative_ai_payload = build_honest_ai_lattice_from_induction(
        double_publication_library_induction,
        unknown_ordering=publication_bs_analysis["unknown_ordering"],
    ...
    double_bs_ai_quotient_raw = build_bs_ai_quotient_report(
        publication_bs_analysis,
        double_authoritative_ai_payload,
        unknown_ordering=publication_bs_analysis["unknown_ordering"],
    )
    ...
    double_ai_status = (
        "full_ai_lattice"
        if double_authoritative_ai_payload["new_authoritative_ai_rank"] >= int(publication_bs_analysis["nullity"])
        else (
            "partial_ai_lattice"
    ...
    summary = {
        ...
        "AI_status": {
            ...
            "authoritative_ai_rank": int(double_authoritative_ai_payload["new_authoritative_ai_rank"]),
            "old_verified_ai_rank": int(double_ai_zero_subset_rank_report["zero_subset_rank"]),
    ...
    lines = [
        ...
        f"`{double_authoritative_ai_payload['new_authoritative_ai_rank']}`.",
        ...
        (
            ...
            else "- The present run upgrades the double path beyond the minimal witness, but the honest published-shell double quotient is not yet complete."
        ),
    ]
    return {
        "summary": summary,
        ...
        "authoritative_ai_payload": double_authoritative_ai_payload,
...
def build_next_step_prompt(single: dict[str, Any], double: dict[str, Any], portability_summary: dict[str, Any]) -> str:
    ...
    if double["summary"].get("quotient_status", {}).get("status") == "success":
        next_unique_task = (
            "keep the publication-level C_pub fixed, preserve all SG194/current-setting-specific conversion wording, and use the extracted single/double quotients for cross-check, closeout, or downstream packaging rather than reopening BS/AI completion."
...
def build_package_readme() -> str:
    ...
    return "\n".join(
        [
            ...
            f"27. {AI_VS_BILBAO_ALIGNMENT_MD.relative_to(ROOT)}",
            f"28. {P4_INDUCTION_FAILURE_MD.relative_to(ROOT)}",
            f"29. {P4_EXACT_SOLVER_RELIABILITY_MD.relative_to(ROOT)}",
            f"30. {P4_BAND_CHARACTER_PHASE_MD.relative_to(ROOT)}",
            f"31. {P4_TRACE_FORMULA_EXPLICIT_MD.relative_to(ROOT)}",
            f"32. {CHARACTER_FIELD_BASIS_CONVENTION_AUDIT_MD.relative_to(ROOT)}",
            f"33. {SG194_SETTING_SPECIFIC_CHARACTER_CONVERSION_VALIDATION_MD.relative_to(ROOT)}",
            f"34. {CHARACTER_FIELD_CONVERSION_GLOBAL_VALIDATION_MD.relative_to(ROOT)}",
            f"35. {P4_CONVERSION_PATCH_INDEPENDENT_VALIDATION_MD.relative_to(ROOT)}",
            f"36. {CLAIM_SCOPE_GUARDRAIL_MD.relative_to(ROOT)}",
            f"37. {D3H_LIKE_LOCAL_OBJECT_CROSSCHECK_MD.relative_to(ROOT)}",
            f"38. {PPATH06_OBSTRUCTION_MD.relative_to(ROOT)}",
            f"39. {PPATH06_ROW_SEMANTICS_MD.relative_to(ROOT)}",
            f"40. {AI_ZERO_SUBSET_RANK_MD.relative_to(ROOT)}",
            f"41. {PARTIAL_AI_LATTICE_WITNESS_MD.relative_to(ROOT)}",
            f"42. {AI_HONEST_BLOCKER_MD.relative_to(ROOT)}",
            "",
            "## PDF Report",
            f"- report file: `{REPORT_PDF.name}`",
            f"- report source: `{REPORT_TEX.name}`",
