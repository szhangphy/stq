...
def build_geometry_summary(spec: GroupSpec, adapter, artifacts: dict[str, Any]) -> dict[str, Any]:
    if _is_generic_spec(spec):
        ...
        object_counts = _count_object_kinds(kspace["entries"])
        return {
            "generated_at": now_iso(),
            "group": spec.group_id,
            "status": "built_from_trusted_symmetry_ops",
            "shared_backbone_available": True,
            "shared_backbone_mode": "generic_from_swyckoff_symmetry_ops",
            "coordinate_system": kspace["coordinate_system"],
            ...
            "generic_builder_layers": {
                ...
                "quotient_prerequisites": "available",
            },
        }
