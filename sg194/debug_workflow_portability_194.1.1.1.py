...
import swyckoff_k
import swyckoff_r
from SSGReps.SSGReps.SG_utils import identify_SG_from_symmetry
...
def coordinate_to_id_map(points: Sequence[dict]) -> dict[tuple[str, str, str], str]:
    return {vector_key(point["_anchor"]): point["id"] for point in points}
...
def augment_connectivity_with_boundary_points(kgeom: dict[str, Any], synthetic_points: list[dict[str, Any]]) -> None:
    id_by_coord = {tuple(point["sample_point"]): point["id"] for point in synthetic_points}
    for point in kgeom["grouped"]["points"]:
        id_by_coord[tuple([frac_str(mod1_fraction(value)) for value in point["_anchor"]])] = point["id"]
...
def build_line_block(line_obj: dict[str, Any], captures: dict[str, Any]) -> dict[str, Any]:
    line_id = line_obj["id"]
    endpoint_ids = [endpoint["point_id"] for endpoint in line_obj["endpoints"]]
    ...
    for endpoint_id in endpoint_ids:
        endpoint_raw = captures[endpoint_id]
        unitary_map = {
...
def derive_plane_corner_ids(plane_obj: dict[str, Any], point_by_coord: dict[tuple[str, str, str], str]) -> list[str]:
    ...
    for coeff1 in BOUNDARY_VALUES:
        for coeff2 in BOUNDARY_VALUES:
            ...
            out.append(point_by_coord[vector_key(point)])
    return sorted(dict.fromkeys(out), key=lambda item: (item[0], int(item[1:])))


def build_plane_block(plane_obj: dict[str, Any], corner_ids: list[str], captures: dict[str, Any]) -> dict[str, Any]:
    ...
    plane_basis_matrix = sp.Matrix([[as_exact_char(value) for value in rep_character] for rep_character in plane_raw["character"]]).T
    local_unknown_ordering: list[str] = []
    corner_decompositions: dict[str, Any] = {}
    equations = []
    matrix_rows = []
    for point_id in corner_ids:
        point_raw = captures[point_id]
        unitary_map = {
...
def induce_candidate(entry: dict[str, Any], local_character: dict[int, complex], ctx: dict[str, Any], captures: dict[str, Any], unknown_ordering: list[str], global_matrix: list[list[int]]) -> dict[str, Any]:
    ...
    for token in unknown_ordering:
        ...
        unknown_vector.append(manifold_multiplicities[manifold_id][int(rep_index_str) - 1])
...
def build_single_pilot(module: Any, ssg_dict: dict[str, Any]) -> dict[str, Any]:
    ...
    line_blocks = [build_line_block(line, captures) for line in grouped["lines"]]
    ...
    plane_blocks = [build_plane_block(plane, derive_plane_corner_ids(plane, point_by_coord), captures) for plane in grouped["planes"]]
    ...
    for entry in ctx["wyckoff_entries"]:
        ...
        candidate = induce_candidate(entry, local_char, ctx, captures, bs_analysis["unknown_ordering"], with_planes["global_matrix"])
    ...
    return {"summary": summary, "audit_text": "\n".join(lines), "kgeom": kgeom, "captures": captures, "with_planes": with_planes, "bs_analysis": bs_analysis, "ai_candidates": ai_candidates}


def build_double_pilot(module: Any, ssg_dict: dict[str, Any], single_kgeom: dict[str, Any]) -> dict[str, Any]:
    ...
    line_blocks = [build_line_block(line, captures) for line in single_kgeom["grouped"]["lines"]]
    line_full = build_global_compatibility(line_blocks, point_ids)
    point_by_coord = build_synthetic_kpoint_map(single_kgeom)
    for point in single_kgeom["grouped"]["points"]:
        point_by_coord[tuple(point["sample_point"])] = point["id"]
    print("[pilot] double: plane blocks")
    plane_blocks = [build_plane_block(plane, derive_plane_corner_ids(plane, point_by_coord), captures) for plane in single_kgeom["grouped"]["planes"]]
    with_planes = build_with_planes_compatibility(line_full, plane_blocks)
    bs_analysis = analyze_kernel(with_planes)
    ...
    minimal = induce_candidate(generic_entry, local_char, ctx, captures, bs_analysis["unknown_ordering"], with_planes["global_matrix"])
    ...
    return {"summary": summary, "audit_text": "\n".join(lines), "with_planes": with_planes, "bs_analysis": bs_analysis, "minimal": minimal}
