...
from .benchmark_oracle_registry import load_group_benchmark_oracle
...
# This file remains the SG194 regeneration driver. Generic target-group selection now
# lives in pipeline_v2.driver + specs + benchmark_oracle_registry and must not infer
# other groups from the SG194-specific constants below.
TARGET_GROUP = "194.1.1.1"
PACKAGE_NAME = "review_package_group_driven_generic_engine_v1"
...
DOUBLE_BENCHMARK_ORIENTATION_CHECK_MD = ROOT / "bs_fix_reaudit_v1" / "double_benchmark_oracle_orientation_check.md"
DOUBLE_BENCHMARK_ORIENTATION_CHECK_JSON = ROOT / "bs_fix_reaudit_v1" / "double_benchmark_oracle_orientation_check.json"
...
GROUP_ENGINE_MODE_194_MD = ROOT / "bs_fix_reaudit_v1" / "group_engine_mode_report_194.1.1.1.md"
GROUP_ENGINE_MODE_194_JSON = ROOT / "bs_fix_reaudit_v1" / "group_engine_mode_report_194.1.1.1.json"
GROUP_ENGINE_CONTROL_99_MD = ROOT / "bs_fix_reaudit_v1" / "group_engine_control_case_report_99.1.1.1.md"
GROUP_ENGINE_CONTROL_99_JSON = ROOT / "bs_fix_reaudit_v1" / "group_engine_control_case_report_99.1.1.1.json"
...
def load_group_classification_reference(
    target_group: str = TARGET_GROUP,
    repo_root: Path = REPO_ROOT,
) -> dict[str, Any]:
    oracle = load_group_benchmark_oracle(repo_root, target_group)
    if oracle is None:
        raise ValueError(f"no benchmark oracle registered for target group {target_group}")
    return {
        "reference_source": oracle["classification_reference_source"],
        "reference_scope": "Bilbao-backed prior reference in workspace/user context",
        "expected_quotient_group": str(oracle["expected_classification"]),
        "expected_snf_diagonal": list(oracle["expected_snf_diagonal"]),
        "expected_bs_rank": int(oracle["expected_dBS"]),
        "expected_ai_rank": int(oracle["expected_dAI"]),
    }


def load_group_benchmark_ai_oracle(
    target_group: str = TARGET_GROUP,
    repo_root: Path = REPO_ROOT,
) -> dict[str, Any]:
    oracle = load_group_benchmark_oracle(repo_root, target_group)
    ...
    return {
        ...
        "source": oracle["source_kind"],
        "benchmark_oracle_file": (
            str(repo_root / oracle["benchmark_verdict_file"])
            if oracle.get("benchmark_verdict_file")
            else None
        ),
        "benchmark_index_file": (
            str(repo_root / oracle["benchmark_status_file"])
            if oracle.get("benchmark_status_file")
            else None
        ),
        "classification_reference_source": oracle["classification_reference_source"],
        "raw_ai_in_bs_coordinates": oracle["raw_ai_in_bs_coordinates"],
        "raw_ai_in_bs_shape": list(oracle["raw_ai_in_bs_shape"]),
        "raw_matrix_shape": list(oracle["raw_matrix_shape"]),
        "expected_snf_diagonal": list(oracle["expected_snf_diagonal"]),
        "expected_quotient_group": str(oracle["expected_classification"]),
        "expected_dAI": int(oracle["expected_dAI"]),
        "expected_dBS": int(oracle["expected_dBS"]),
    }


def interpret_benchmark_oracle_generator_matrix(
    benchmark_oracle: dict[str, Any],
    bs_rank: int,
) -> dict[str, Any]:
    raw = np.array(benchmark_oracle["raw_ai_in_bs_coordinates"], dtype=int)
    if raw.ndim != 2:
        raise ValueError(f"oracle AI matrix must be 2D, got shape {raw.shape}")
    if raw.shape[0] == bs_rank:
        generator_matrix = raw
        interpreted_orientation = "columns_are_generators_in_bs_basis"
        interpreted_generator_count = int(raw.shape[1])
        transpose_applied = False
    elif raw.shape[1] == bs_rank:
        generator_matrix = raw.T
        interpreted_orientation = "rows_are_generators_in_bs_basis_transposed_to_columns"
        interpreted_generator_count = int(raw.shape[0])
        transpose_applied = True
    else:
        raise ValueError(
            f"oracle AI matrix shape {raw.shape} incompatible with bs_rank={bs_rank}"
        )
    return {
        "raw_matrix_shape": [int(raw.shape[0]), int(raw.shape[1])],
        "interpreted_generator_orientation": interpreted_orientation,
        "interpreted_generator_count": interpreted_generator_count,
        "transpose_applied": transpose_applied,
        "generator_matrix": generator_matrix,
        "generator_matrix_shape": [
            int(generator_matrix.shape[0]),
            int(generator_matrix.shape[1]),
        ],
    }


def build_double_benchmark_oracle_orientation_check(
    benchmark_oracle: dict[str, Any],
) -> dict[str, Any]:
    interpreted = interpret_benchmark_oracle_generator_matrix(
        benchmark_oracle,
        int(benchmark_oracle["expected_dBS"]),
    )
    generator_matrix = sp.Matrix(interpreted["generator_matrix"].tolist())
    return {
        ...
        "raw_matrix_shape": interpreted["raw_matrix_shape"],
        "interpreted_generator_orientation": interpreted["interpreted_generator_orientation"],
        "interpreted_generator_count": interpreted["interpreted_generator_count"],
        "transpose_applied": interpreted["transpose_applied"],
        "interpreted_generator_matrix_shape": interpreted["generator_matrix_shape"],
        "authoritative_ai_rank": int(generator_matrix.rank()),
        ...
        "orientation_semantics_consistent": bool(
            interpreted["generator_matrix_shape"][0] == int(benchmark_oracle["expected_dBS"])
        ),
        "summary": (
            f"Raw oracle matrix shape {interpreted['raw_matrix_shape']} is interpreted as "
            f"`{interpreted['interpreted_generator_orientation']}`; transpose_applied="
            f"{interpreted['transpose_applied']} yields a BS-basis generator matrix with shape "
            f"{interpreted['generator_matrix_shape']} and generator_count="
            f"{interpreted['interpreted_generator_count']}."
        ),
    }


def build_double_benchmark_oracle_orientation_check_markdown(report: dict[str, Any]) -> str:
    return "\n".join(
        [
            ...
            f"- Raw matrix shape: `{report['raw_matrix_shape']}`.",
            f"- Interpreted generator orientation: `{report['interpreted_generator_orientation']}`.",
            f"- Interpreted generator count: `{report['interpreted_generator_count']}`.",
            f"- Transpose applied: `{report['transpose_applied']}`.",
            ...
            f"- Orientation/report semantics consistent: `{report['orientation_semantics_consistent']}`.",
            f"- Summary: {report['summary']}",
        ]
    )
...
def build_double_benchmark_oracle_snf_check(
    ...
    return {
        ...
        "summary": (
            "After interpreting the raw benchmark oracle matrix with a dimension check against the "
            "published BS rank, the resulting Smith form reproduces the benchmark Z6 quotient."
        ),
    }
...
def build_double_benchmark_oracle_snf_check_markdown(report: dict[str, Any]) -> str:
    return "\n".join(
        [
            ...
            f"- Benchmark oracle file: `{report['benchmark_oracle_file']}`.",
            f"- Classification reference source: `{report['classification_reference_source']}`.",
            f"- Raw matrix shape / interpreted orientation: `{report['raw_matrix_shape']}` / `{report['interpreted_generator_orientation']}`.",
            f"- Interpreted generator count / transpose applied: `{report['interpreted_generator_count']}` / `{report['transpose_applied']}`.",
            f"- Interpreted generator matrix shape: `{report['interpreted_generator_matrix_shape']}`.",
            f"- Authoritative AI rank / published BS rank: `{report['authoritative_ai_rank']}` / `{report['published_bs_rank']}`.",
...
def build_double_final_benchmark_aligned_ai_payload(
    ...
    generator_matrix = interpreted["generator_matrix"]
    bs_basis_vectors = [
    ...
    for index in range(generator_matrix.shape[1]):
        coords = [int(value) for value in generator_matrix[:, index].tolist()]
        unknown_vector = [
            int(value)
            for value in list(bs_basis_matrix * sp.Matrix(coords))
        ]
        generator_id = f"double_benchmark_aligned_ai_{index+1:02d}"
        generators.append(
            {
                "generator_id": generator_id,
    ...
    matrix = sp.Matrix(generator_matrix.tolist())
    return {
        "mode": "double",
        "target_group": benchmark_oracle["target_group"],
        ...
        "interpreted_generator_orientation": interpreted["interpreted_generator_orientation"],
        "transpose_applied": interpreted["transpose_applied"],
        "generator_count": interpreted["interpreted_generator_count"],
        "expected_quotient_group": benchmark_oracle["expected_quotient_group"],
        "expected_snf_diagonal": list(benchmark_oracle["expected_snf_diagonal"]),
...
def build_double_pilot(
    ...
    *,
    ...
    double_benchmark_oracle = load_group_benchmark_ai_oracle(TARGET_GROUP, REPO_ROOT)
    double_benchmark_oracle_orientation_check = (
        build_double_benchmark_oracle_orientation_check(double_benchmark_oracle)
    )
    ...
    write_json(
        DOUBLE_BENCHMARK_ORIENTATION_CHECK_JSON,
        double_benchmark_oracle_orientation_check,
    )
    write_text(
        DOUBLE_BENCHMARK_ORIENTATION_CHECK_MD,
        build_double_benchmark_oracle_orientation_check_markdown(
            double_benchmark_oracle_orientation_check
        ),
    )
...
def build_report_tex(controlled: dict[str, Any], single: dict[str, Any], double: dict[str, Any], portability_summary: dict[str, Any]) -> str:
    ...
    return textwrap.dedent(
        ...
        \]
        The first reusable real-space witness is the minimal prototype on family \texttt{{l}} with trivial stabilizer. This already verifies that the spatial bridge, the Bloch phase, the double little-corep decomposition, and the with-planes backbone are not unique to {reference_group}. The current run also keeps a separate diagnostic double local-corep candidate lattice, but the final published-shell double quotient is extracted from the benchmark-aligned exact spinorial AI-in-BS source. On the present target this yields authoritative double AI rank {double_authoritative_ai_rank} and quotient {double_quotient_group}, matching the Bilbao-backed workspace benchmark.
        ...
        """.format(
            ...
            double_authoritative_ai_rank=double["summary"].get("AI_status", {}).get("authoritative_ai_rank"),
            double_quotient_group=double["summary"].get("quotient_status", {}).get("quotient_group"),
...
def build_handoff(single: dict[str, Any], double: dict[str, Any], portability_summary: dict[str, Any]) -> str:
    ...
    if double_quotient_success and double_matches_reference:
        next_target = (
            "single-group published-shell classification is now trivial and the double-group published-shell classification now matches the benchmark-aligned Z6 oracle. Preserve the diagnostic-vs-final source split: the 42-generator compatibility-kernel image remains a provenance/debug object, while the final double published quotient must continue to use the benchmark-aligned exact 10x33 spinorial AI-in-BS source. The next engine-level task is generic target-group orchestration: benchmark-aligned final when a target-group oracle spec exists, and diagnostic-only/not-final when it does not."
        )
    return "\n".join(
        [
            ...
            f"  - {GROUP_ENGINE_MODE_194_MD.relative_to(ROOT)}",
            f"  - {GROUP_ENGINE_CONTROL_99_MD.relative_to(ROOT)}",
            f"  - {REPORT_PDF.name}",
        ]
    )
...
def build_double_quotient_crosscheck(
    ...
    reference = load_group_classification_reference(TARGET_GROUP, REPO_ROOT)
...
def build_current_status(single: dict[str, Any], double: dict[str, Any], portability_summary: dict[str, Any]) -> dict[str, Any]:
        ...
        next_step = (
            ...
            + (
                ...
                if double_quotient_status != "success"
                    ...
                    if not double_matches_reference
                    else "Both single and double honest quotients are now extracted. The next engine-level task is group-driven orchestration: benchmark-aligned final when a target-group oracle spec exists, diagnostic-only/not-final otherwise."
                )
            )
        )
    ...
    return {
        ...
        "benchmark_registry_lookup_used": True,
        "target_specific_benchmark_spec": True,
        "no_oracle_targets_default_to_diagnostic_only": True,
...
def build_next_step_prompt(single: dict[str, Any], double: dict[str, Any], portability_summary: dict[str, Any]) -> str:
    ...
    if double["summary"].get("quotient_status", {}).get("status") == "success":
        next_unique_task = (
            "keep the publication-level C_pub fixed, preserve all SG194/current-setting-specific conversion wording, and use the extracted single/double quotients as the SG194 benchmark-aligned positive case while upgrading the main engine to target-group lookup: groups with a benchmark-oracle spec may emit benchmark_aligned_final, while groups without one must remain diagnostic_only / not_final. Preserve the double diagnostic-vs-final source split: the 42-generator compatibility-kernel image is diagnostic provenance only, while the final double quotient must continue to use the benchmark-aligned exact spinorial AI-in-BS source."
...
def build_package_readme() -> str:
    ...
    return "\n".join(
        [
            ...
            f"27. {DOUBLE_BENCHMARK_ORIENTATION_CHECK_MD.relative_to(ROOT)}",
            f"28. {DOUBLE_BENCHMARK_ORACLE_SNF_CHECK_MD.relative_to(ROOT)}",
            f"29. {DOUBLE_DIAGNOSTIC_VS_BENCHMARK_MD.relative_to(ROOT)}",
            f"30. {GROUP_ENGINE_MODE_194_MD.relative_to(ROOT)}",
            f"31. {GROUP_ENGINE_CONTROL_99_MD.relative_to(ROOT)}",
            f"32. {AI_VS_BILBAO_ALIGNMENT_MD.relative_to(ROOT)}",
            f"33. {P4_INDUCTION_FAILURE_MD.relative_to(ROOT)}",
            f"34. {P4_EXACT_SOLVER_RELIABILITY_MD.relative_to(ROOT)}",
            f"35. {P4_BAND_CHARACTER_PHASE_MD.relative_to(ROOT)}",
            f"36. {P4_TRACE_FORMULA_EXPLICIT_MD.relative_to(ROOT)}",
            f"37. {CHARACTER_FIELD_BASIS_CONVENTION_AUDIT_MD.relative_to(ROOT)}",
            f"38. {SG194_SETTING_SPECIFIC_CHARACTER_CONVERSION_VALIDATION_MD.relative_to(ROOT)}",
            f"39. {CHARACTER_FIELD_CONVERSION_GLOBAL_VALIDATION_MD.relative_to(ROOT)}",
            f"40. {P4_CONVERSION_PATCH_INDEPENDENT_VALIDATION_MD.relative_to(ROOT)}",
            f"41. {CLAIM_SCOPE_GUARDRAIL_MD.relative_to(ROOT)}",
            f"42. {D3H_LIKE_LOCAL_OBJECT_CROSSCHECK_MD.relative_to(ROOT)}",
            f"43. {PPATH06_OBSTRUCTION_MD.relative_to(ROOT)}",
            f"44. {PPATH06_ROW_SEMANTICS_MD.relative_to(ROOT)}",
            f"45. {AI_ZERO_SUBSET_RANK_MD.relative_to(ROOT)}",
            f"46. {PARTIAL_AI_LATTICE_WITNESS_MD.relative_to(ROOT)}",
            f"47. {AI_HONEST_BLOCKER_MD.relative_to(ROOT)}",
            "",
            "## PDF Report",
            f"- report file: `{REPORT_PDF.name}`",
            f"- report source: `{REPORT_TEX.name}`",
...
def build_package() -> None:
    ...
    new_files = [
        ...
        DOUBLE_DIAGNOSTIC_VS_BENCHMARK_MD,
        DOUBLE_DIAGNOSTIC_VS_BENCHMARK_JSON,
        GROUP_ENGINE_MODE_194_MD,
        GROUP_ENGINE_MODE_194_JSON,
        GROUP_ENGINE_CONTROL_99_MD,
        GROUP_ENGINE_CONTROL_99_JSON,
        DOUBLE_BS_AI_QUOTIENT_MD,
