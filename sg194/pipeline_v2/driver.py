...
def run_pipeline(config: PipelineRunConfig, repo_root: Path) -> dict[str, Any]:
    ...
    if config.validate:
        ...
        if spec.capabilities.get("final_results") and not checks["final_results_available"]:
            raise SystemExit("pipeline final results are not available")
        if spec.capabilities.get("final_results") and not checks["final_results_verified"]:
            raise SystemExit("pipeline final results are not verified")
    return result
...
def main() -> None:
    ...
    print(
        ...
        f"checks_passed={result['checks']['checks_passed']} "
        f"final_results_available={result['checks']['final_results_available']} "
        f"final_results_verified={result['checks']['final_results_verified']} "
        f"trust={result['spec']['trust_level']} output_dir={output_dir}"
    )
