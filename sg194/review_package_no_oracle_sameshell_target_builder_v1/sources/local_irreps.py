...
def runtime_library_policy(target_group: str) -> dict[str, Any]:
    output_paths = _output_paths(target_group)
    return {
        "target_group": target_group,
        "target_group_slug": _slug_group_id(target_group),
        "uses_default_sg194_paths": target_group == DEFAULT_TARGET_GROUP,
        "output_paths": {key: str(path.relative_to(ROOT)) for key, path in output_paths.items()},
    }
...
def generate_outputs(
    *,
    ...
    payload["runtime_library_policy"] = runtime_library_policy(target_group)
    return payload
