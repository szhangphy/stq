...
def _build_generic_same_shell_target_object(
    *,
    ...
    exact_alignment_status = (
        ...
        else same_shell_quotient.get("blocker_stage")
        or projected_point_shell_attempt.get("blocker_stage")
        or "generic_same_shell_target_blocked"
    )
    blocker = None
    blocker_stage = None
    target_alignment_builder_status = "available" if same_shell_available else "blocked"
    direct_quotient_status = (
        "generic_same_shell_direct_quotient_success"
        if same_shell_available
        else same_shell_quotient.get("blocker_stage")
        or projected_point_shell_attempt.get("blocker_stage")
        or "generic_same_shell_direct_quotient_blocked"
    )
    if not same_shell_available:
        blocker_stage = (
            same_shell_quotient.get("blocker_stage")
            or projected_point_shell_attempt.get("blocker_stage")
            or "generic_same_shell_target_blocked"
        )
        blocker = (
            same_shell_quotient.get("blocker")
    ...
    return {
        ...
        "dbs_dai_gap": same_shell_quotient.get("dbs_dai_gap"),
        "free_rank": same_shell_quotient.get("free_rank"),
        "finite_part": list(same_shell_quotient.get("finite_part", [])),
        "classification": same_shell_quotient.get("classification"),
        "smith_diagonal_nonzero": list(same_shell_quotient.get("smith_diagonal_nonzero", [])),
        ...
        "evidence": {
            "same_shell_quotient": {
                "availability": same_shell_quotient.get("availability"),
                "dBS": same_shell_quotient.get("dBS"),
                "dAI": same_shell_quotient.get("dAI"),
                "classification": same_shell_quotient.get("classification"),
                "free_rank": same_shell_quotient.get("free_rank"),
                "finite_part": list(same_shell_quotient.get("finite_part", [])),
                "smith_diagonal_nonzero": list(same_shell_quotient.get("smith_diagonal_nonzero", [])),
                "ai_candidate_count": same_shell_quotient.get("ai_candidate_count"),
                "ai_candidate_count_used": same_shell_quotient.get("ai_candidate_count_used"),
                "ai_failure_count": same_shell_quotient.get("ai_failure_count"),
                "ai_incompatible_count": same_shell_quotient.get("ai_incompatible_count"),
                "ai_embedding_failure_count": same_shell_quotient.get("ai_embedding_failure_count"),
            },
            "projected_point_shell_attempt": projected_point_shell_attempt,
        },
        "quotient": same_shell_quotient,
    }
...
def generic_mode_progress(group_id: str, mode: str, builder_variant: str = "authoritative") -> dict[str, Any]:
    ...
    if same_shell_target["availability"] == "available":
        return _generic_progress_payload(
            group_id,
            mode,
            builder_variant,
            ...
            generic_published_classification_ready=bool(
                same_shell_target.get("generic_published_classification_ready", False)
            ),
        )
...
def generic_alignment_summary(group_id: str) -> dict[str, Any]:
    ...
    def target_payload(progress: dict[str, Any]) -> dict[str, Any]:
        ...
        projected = (target.get("evidence") or {}).get("projected_point_shell_attempt", {})
        return {
