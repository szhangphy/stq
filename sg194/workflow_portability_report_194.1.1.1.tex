...
\operatorname{rank} C_{\mathrm{single}} = 23, \qquad
\operatorname{nullity} C_{\mathrm{single}} = 11.
\]
The script had to add 0 synthetic 0D boundary points because the raw special-point list is not connectivity-complete for this target. The present AI side is only partial: 12 trivial-family generators span rank 11. This means the pilot reaches an honest BS computation and an honest atomic prototype, but not yet an honest AI completeness audit or quotient extraction.
...
\operatorname{rank} C_{\mathrm{double}} = 23, \qquad
\operatorname{nullity} C_{\mathrm{double}} = 11.
\]
