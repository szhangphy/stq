            ...
            "\n".join(
                [
                    ...
                    *(
                        _md_list(
                            [
                                f"{item['object_id']}: `{item[field]}` in `{item['row_language_level']}` / `{item['object_kind']}`"
        ...
        "\n".join(
            [
                ...
                *(
                    _md_list(
                        [
                            f"{item['object_id']}: `{item['classification']}` via `{item['quotient_derivation_mode']}`"
                    ...
                    _md_list(
                        [
                            (
                                ...
                                f"`{final_status['single_final']['dBS']}/{final_status['single_final']['dAI']}/{final_status['single_final']['classification']}`"
                                if final_status.get("single_final", {}).get("dBS") is not None
                                else f"single final object: `{final_status['single_final']['object_kind']}`"
                            ),
                            (
                                f"double final object: `{final_status['double_final']['object_kind']}` -> "
                                f"`{final_status['double_final']['dBS']}/{final_status['double_final']['dAI']}/{final_status['double_final']['classification']}`"
                                if final_status.get("double_final", {}).get("dBS") is not None
                                else f"double final object: `{final_status['double_final']['object_kind']}`"
                            ),
                            f"same final object: `{final_status['same_final_object']}`",
