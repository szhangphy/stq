...
def build_stage2_summary(
    ...
    return {
        ...
        "interpretation_warning": (
            "The raw internal quotients are preserved as provenance, and the final SG194 ordinary standard quotients are now computed through an externally anchored current-to-standard elimination contract rather than an internal ambient row-space identity proof."
        ),
...
\]
These union ranks are larger than 13, so the external ordinary 13-generator span is not already the identical subspace of the current 34-row ambient shell. The final 13-dimensional standard layer is reached only after external anchoring and elimination of the three common free directions.
...
- single_final_rank_ai = {stage2_summary['single_final_rank_ai']}
        ...
        The next unique task is: review the externally anchored current-to-standard elimination contract, confirm that the union-rank-17 evidence is not being overstated as an ambient identity proof, and verify that the authoritative stage-2 outputs now cite final BS = 13 and final quotient = trivial.
        """
    ).strip()
...
def build_rolling_checkpoint_payload(
    ...
    confirmed_findings = [
        ...
        f"Single final rank(BS) = {single_summary['final_rank_bs']}, final rank(AI) = {single_summary['final_rank_ai']}, final quotient = {single_summary['quotient_group']}.",
        f"Double final rank(BS) = {double_summary['final_rank_bs']}, final rank(AI) = {double_summary['final_rank_ai']}, final quotient = {double_summary['quotient_group']}.",
        f"The extracted-package smoke tests over {PACKAGE_NAME}.tar.gz passed = {smoke_payload['all_passed']}.",
    ]
    return {
        "current_time": current_time,
        "task_scope": "SG194 194.1.1.1 final-standard-space clean closeout for auditability, reproducibility, and self-contained packaging.",
        ...
        "active_blockers": [],
        "next_actions": [
            "Review the git diff for in-scope v3 closeout changes only.",
...
def build_rolling_checkpoint_report(payload: dict[str, Any]) -> str:
    ...
    return textwrap.dedent(
        f"""
        # SG194 194.1.1.1 Final Standard-Space Clean Closeout Report v1
...
def build_rolling_checkpoint_prompt(payload: dict[str, Any]) -> str:
    return textwrap.dedent(
        f"""
        Resume the SG194 194.1.1.1 final-standard-space clean closeout from the current repository root.
        ...
        - single final rank(BS) = 13
        - single final rank(AI) = 13
        - single final quotient = trivial
        - double final rank(BS) = 13
        - double final rank(AI) = 13
        - double final quotient = trivial
        ...
        2. git add the v3 clean-closeout source and generated outputs
