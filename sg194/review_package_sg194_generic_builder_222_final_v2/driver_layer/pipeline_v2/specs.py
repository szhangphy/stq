...
GROUP222_SPEC = GroupSpec(
    key="ssg222_1_1_1",
    group_id="222.1.1.1",
    title="222.1.1.1 trusted-symmetry generic direct-final pipeline",
    description=(
        "Real second-group spec under a strict trust policy: only symmetry-operation evidence "
        "is trusted. Generic builders must construct geometry/current-row-shell/compatibility/"
        "AI/quotient objects directly from those operations before any final BS/AI/quotient claim."
    ),
    home_subdir="sg194",
    ...
    readiness_note=(
        "This spec is real and runnable. The pipeline trusts only the symmetry operations, keeps "
        "core computation in coordinate system B, and now computes direct final single/double "
        "results through generic compatibility/local-AI/quotient builders."
    ),
)
