#!/usr/bin/env python3
"""
Accelerated Wyckoff / k-space special points enumerator.

Key speedups vs the original:
  1) FIXED: solve_congruence() caching was inside the function => never hit.
     Now SNF + congruence solver are cached globally and persist across calls.
  2) PRUNE: before calling expensive solve_congruence on intersections, do a quick
     rank test (rref rank increase) to see if dimension could drop.
  3) ORDER: intersecting ops are ordered by constraint strength (rank/rows) so the
     search tree collapses early and branches less.
  4) DEFAULT FAST: skip full orbit/star expansion unless explicitly requested.

Dependencies:
  - Python stdlib
  - numpy (as in your original script)
  - optional sympy for faster Smith normal form
  - optional irssg when using --irssg-ssgnum
"""
...
GEOMETRY_ONLY_DROP_FIELDS = frozenset(
    {
        "arms",
        "arms_conv",
        "moment_basis",
...
def compact_geometry_entries(
    wyckoff: List[dict],
    *,
    drop_fields: Optional[Sequence[str]] = None,
) -> List[dict]:
    """
    Return a geometry-only projection of Wyckoff/k-manifold entries.

    This keeps the affine subspace data (`dim`, `x0`, `basis_vecs`, `rep`, `orbit`,
    `representative_coordinate`) while dropping spin/moment/site-symmetry display
    fields that are irrelevant for pure geometric post-processing.
    """
    drop = set(GEOMETRY_ONLY_DROP_FIELDS)
    if drop_fields:
        drop.update(drop_fields)
    return [_prune_json_fields(entry, drop) for entry in wyckoff]


def build_geometry_json_payload(
    data: Union[dict, str],
    *,
    kspace: bool = False,
    basis: str = "primitive",
    ...
    return {
        ...
        "wyckoff": compact_geometry_entries(wyckoff, drop_fields=drop_fields),
    }
...
def load_irssg_data(ssgnum: str, spin_id: int) -> Tuple[dict, List[bool]]:
    ...
    for data_file in (
        Path(__file__).resolve().parent / "SSGReps" / "ssg_data" / "msg.pkl",
        Path("/data/work/szhang/ssg/msgid/msg.pkl"),
    ):
        if not data_file.exists():
            continue

        def _load_one_ssg_from_pickle(query_ssgnum: str, *, _data_file=data_file):
            import pickle
            ...
            for ssg in ssg_list:
                if ssg.get("ssgNum") == query_ssgnum:
                    return ssg
            raise ValueError(f"Cannot load SSG number {query_ssgnum} from {_data_file}")

        pickle_loaders.append(_load_one_ssg_from_pickle)
    ...
    operation = None
    if load_one_ssg is not None:
        try:
            operation = load_one_ssg(ssgnum)
        except (FileNotFoundError, ValueError) as exc:
            last_error = exc
    if operation is None:
        for loader in pickle_loaders:
            try:
                operation = loader(ssgnum)
                break
    ...
    if operation is None:
        if last_error is not None:
            raise last_error
        raise ImportError("irssg.load_one_ssg is not available in the current environment.")
